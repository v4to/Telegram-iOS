...
//            transition.updatePosition(node: controller, position: CGPoint(x: scaledSize.width / 2.0, y: scaledSize.height / 2 + (size.height - scaledSize.height) / 2))
            transition.updatePosition(node: controller, position: CGPoint(x: scaledSize.width / 2.0, y: scaledSize.height / 2.0))

//            transition.updatePosition(node: controller, position: CGPoint(x: size.width / 2.0, y: scaledSize.height / 2.0))
            ...
            controller.controller.containerLayoutUpdated(ContainerViewLayout(size: CGSize(width: size.width, height: size.height + 20), metrics: LayoutMetrics(widthClass: .compact, heightClass: .compact), deviceMetrics: .iPhoneX, intrinsicInsets: UIEdgeInsets(), safeInsets: UIEdgeInsets(), additionalInsets: UIEdgeInsets(), statusBarHeight: nil, inputHeight: 24, inputHeightIsInteractivellyChanging: false, inVoiceOver: false), transition: transition)
